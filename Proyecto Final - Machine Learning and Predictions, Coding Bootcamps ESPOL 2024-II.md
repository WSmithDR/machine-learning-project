...
* Análisis Multivariado:  
  ...
  * Elija al menos dos variables predictoras numéricas y una variable predictora categórica y realice un análisis multivariable entre las variables elegidas y la variable target.  
...
Suba sus predicciones sobre los datos de test a la carpeta de Google Drive asignada a su equipo, utilizando su mejor modelo

**Entregable 2 (plazo de entrega 29/04/2025)**

Suba a Aula Virtual un (1) Jupyter Notebook con las siguientes tareas:

* En adición a los modelos implementados en el Entregable 1 del proyecto, implemente al menos **un nuevo modelo** considerando un algoritmo distinto (por ej. si en el Entregable 1 implementó un modelo SVM, el modelo del Entregable 2 ya no puede ser ese). Puede utilizar algún algoritmo de clasificación visto en clases o uno que no haya sido visto ([Regresión Logística](https://scikit-learn.org/stable/modules/generated/sklearn.linear_model.LogisticRegression.html), [Naive Bayes](https://scikit-learn.org/stable/modules/naive_bayes.html), etc.). Evalúe sus modelos y describa/interprete sus resultados obtenidos. Compare con sus resultados del Entregable 1\.  
* Escoja **una** **estrategia** aprendida en clases para tratar el desbalance de clases (SMOTE, Random Resampling, scale pos weight, etc.). Entrene 2 modelos utilizando esa estrategia:  uno basado en el mejor modelo del Entregable 1, y otro basado en el modelo elegido para el Entregable 2 en el punto anterior. Note que no todas las estrategias para tratar desbalance de clases se pueden usar con todos los algoritmos (por ej. no se puede usar scale pos weight con SVM); por lo que en esos casos podría elegir una estrategia distinta para cada modelo. Evalúe sus modelos y describa/interprete sus resultados obtenidos.  
* Seleccione el mejor modelo evaluado hasta este punto para realizar optimización de hiperparámetros utilizando Randomized Search u optimización Bayesiana de hiperparámetros (puede usar [optuna](https://optuna.readthedocs.io/en/stable/index.html) para esto). Para ello considere lo siguiente:  
  * Seleccione al menos 3 hiperparámetros a optimizar (hiperparámetros con valores continuos y discretos) y explique su elección.  
  * Independientemente del método de optimización elegido, explore al menos 50 combinaciones de hiperparámetros.  
  * Describa/interprete los resultados obtenidos con el modelo optimizado.  
* **OPCIONAL:** Considere **un registro** cualquiera para el cual su mejor modelo indica que el cliente cometió fraude (clase positiva). El equipo legal de la compañía “KLEY S.A.” iniciará acciones legales en contra del cliente y desea saber porqué el modelo implementado por su equipo de científicos de datos considera que dicho cliente cometió fraude. Utilizando e interpretando los obtenido con SHAP, describa una explicación con la cual el equipo legal de la compañía basará sus acciones.
